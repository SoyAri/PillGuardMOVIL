...
import { fetchAiComment } from "../chatgptconfig"; // Función para obtener comentarios generados por IA
...
export default function HomeScreen() {
  ...
  // useEffect para cargar y actualizar el tema de la app
  useEffect(() => {
    fetchTheme(setTema);

    // Detecta cambios en AsyncStorage para el tema
    const handleStorageChange = async () => {
      fetchTheme(setTema);
    };

    AsyncStorage.getAllKeys((err, keys) => {
      if (keys && keys.includes('tema')) {
        AsyncStorage.getItem('tema', (err, result) => {
          if (result) {
            setTema(result);
          }
        });
      ...
      >
        ...
        {notificationVisible && (
          <View style={[
            ...
            <Text style={[styles.notificationText, tema === 'claro' && styles.notificationTextClaro]}>
              Fecha de inicio: {getFormattedDate(selectedPill?.startDate)}
            </Text>
            {selectedPill?.endDate && (
              <Text style={[styles.notificationText, tema === 'claro' && styles.notificationTextClaro]}>
                Fecha de fin: {getFormattedDate(selectedPill?.endDate)}
              </Text>
            )}
            <Text style={[styles.notificationText, tema === 'claro' && styles.notificationTextClaro]}>
              Intervalo de horas: {selectedPill?.intervalHours}
            </Text>
            <Text style={[styles.notificationText, tema === 'claro' && styles.notificationTextClaro]}>
